...
export const getCachedFeed = () =>
  localforage.getItem(FEED_CACHE_NAME).then((value) => value || []);

export const cacheFeedItem = (feedItem) =>
  localforage.getItem('feed').then((value) => {
...
export const clearMediaCache = () =>
  localforage.getItem('feed').then((value) => {
    const comicUrls = value
      ? value.filter((f) => f.type === FEED_TYPE_COMIC).map((c) => c.content)
      : [];
    return caches
      .open('dailyRusk-media')
      .then((cache) =>
        cache
          .keys()
          .then((keys) =>
            Promise.all(
              keys
                .filter((k) => comicUrls.indexOf(k.url) === -1)
